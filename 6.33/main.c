/* 6.33
 * This program will modify the program of Fig. 6.15 to use a recursive binarySearch function to perform
 * the binary search of an array. The function will receive an integer array, the starting subscript, the
 * ending subscript, and the search key as arguments. If the search key is found, return the array subscript;
 * otherwise, return -1.
 */
...
int binarySearch(const int b[], int key, size_t low, size_t high) {
   ...
   if (low <= high) {
      ...
      if (key == b[middle]) {
       ...
       // if key < b[middle], function calls itself again with middle reduced by 1
      if (key < b[middle]) {
         return binarySearch(b, key, low, middle - 1);
      }
      // if key is still not found, function calls itself with middle increased by 1
