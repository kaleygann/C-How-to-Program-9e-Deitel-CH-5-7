/* 6.34
 * Eight Queens (Recursion): this program will solve the Eight Queens problem
...
int solveQNs(int col) {
    ...
    for (int row = 0; row < SIZE; ++row) {
        if (elimination_numbers[row][col] > 0 && isSafe(row, col)) {

            // place queen in the row
            queens[col] = row;
            updateElimination(row, col, -1); // update elimination numbers, decrement value
            ...
            // backtrack if queen placement does not create a solution
                ...
                updateElimination(row, col, 1);  // update elimination numbers, increment value (restore number)
            }
        }
    }  // end for
