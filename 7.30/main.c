/* 7.30
 * This program will modify the program in 7.29 to make
 * enhancements the software-based simulation of the Simpletron
 * computer. This version adds newline, exponentiation, remainder operations,
 * floating point values, and increasing memory size to 1000 */
...
// function prototypes
void load(float memory[]);
void execute(float memory[], float *accumulator, int *instructionCounter, int *instructionRegister,
    ...
    while (*instructionCounter < LIMIT) {
        ...
        *operand = *instructionRegister % 100;
        ...
        switch (*operationCode) {
            ...
            case REMAINDER: // divide a value from a specific location in memory into accumulator value and get remainder
                printf("REMAINDER: %f\n", memory[*operand]);
                if (memory[*operand] == 0) {
                    printf("\n*** Attempt to divide by zero ***\n");
                    printf("\n*** Simpletron execution abnormally terminated ***\n");
            ...
            case EXPONENT:  // exponentiate accumulator by a value from a location in memory
